...
/// user, so screens call e.g. `actions.saveProduct(p)` without threading ids.
class InventoryActions {
  InventoryActions(this._repo, this._companyId, this._userId);
  ...
  Future<String> createProduct(Product product, {num openingStock = 0}) {
    return _repo.createProduct(_companyId, product,
        openingStock: openingStock, userId: _userId);
  }
...
final inventoryActionsProvider = Provider<InventoryActions>((ref) {
  ...
  return InventoryActions(repo, profile.companyId, profile.uid);
});
