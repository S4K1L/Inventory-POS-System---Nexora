...
/// Most recent sales (newest first) — powers the Sales History screen.
final recentSalesProvider = StreamProvider<List<Sale>>((ref) {
  ...
  return ref.watch(salesRepositoryProvider).watchRecentSales(cid, limit: 100);
});

/// Sales since local midnight — powers "Today's Sales".
final todaySalesProvider = StreamProvider<List<Sale>>((ref) {
  final cid = ref.watch(_companyIdProvider);
  if (cid.isEmpty) return Stream.value(const []);
  final now = DateTime.now();
  final midnight = DateTime(now.year, now.month, now.day);
  return ref.watch(salesRepositoryProvider).watchSalesSince(cid, midnight);
});
...
/// Runs checkout using the current company + user.
final checkoutServiceProvider = Provider<CheckoutService>((ref) {
  final repo = ref.watch(salesRepositoryProvider);
  final profile = ref.watch(currentProfileProvider);
  return CheckoutService(repo, profile.companyId, profile.uid);
});

class CheckoutService {
  CheckoutService(this._repo, this._companyId, this._userId);
